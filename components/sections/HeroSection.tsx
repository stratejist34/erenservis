...
const TRUST_DOTS = [
  { icon: Clock, label: 'Aynı gün inceleme' },
  { icon: FileText, label: 'Yazılı fiyat teklifi' },
  { icon: ShieldCheck, label: '2 yıl işçilik garantisi' },
] as const;
...
export default function HeroSection() {
  return (
    <section className="relative mx-auto grid max-w-7xl gap-10 overflow-hidden px-4 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
      ...
      {/* ===== LAYER 2: Gradient overlay - ORTADA ===== */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-10 pt-10 lg:pt-16">
        ...
        <p className="hero-item-3 mt-7 max-w-[52ch] text-lg leading-8 text-[#94A3B8]">
          Geç kalırsanız onarım maliyeti katlanır. DSG, ZF ve CVT sistemlerde doğru
          teşhisle süreci baştan kontrol altına alıyoruz.
        </p>
        ...
        <div className="hero-item-4 mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          >
            ...
            Ücretsiz Ön Teşhis — Hemen Arayın
          </a>
          <a
            href={WHATSAPP_HREF}
            target="_blank"
        ...
        <div className="hero-item-5 mt-7 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[#64748B]">
          {TRUST_DOTS.map((dot) => (
            <span key={dot.label} className="inline-flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-[#38BDF8] dot-glow-sky" />
              {dot.label}
            </span>
          ))}
        </div>
      ...
      {/* Sağ — Diagnostic Cost Readout (UI Panel) */}
      <div
        className="animate-scan-reveal relative z-10 grid gap-4 lg:max-w-[92%] lg:pt-12"
        style={{ animationDelay: '0.3s' }}
      >
        {/* UI Panel with reflection glow */}
        <div
          className="glass-surface-strong rounded-[28px] p-5"
        >
          {/* Terminal header */}
